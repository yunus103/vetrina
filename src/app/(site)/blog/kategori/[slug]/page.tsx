...
import { blogPostsByCategoryQuery, categoryBySlugQuery } from "@/sanity/lib/queries";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${category.title} Kategorisindeki Yazılar`,
    description: category.description || `${category.title} hakkındaki tüm blog yazılarımız.`,
    canonicalPath: `/blog/kategori/${slug}`,
  });
}
...
export default async function CategoryPage({ params }: Props) {
  ...
  const [category, posts] = await Promise.all([
    ...
    client.fetch(blogPostsByCategoryQuery, { slug }, { next: { tags: ["blog"] } })
  ]);
  ...
  return (
    <div className="min-h-screen bg-backgroundLight pb-24">
      ...
      <div className="pt-32 pb-16 px-6 md:px-12 text-center max-w-7xl mx-auto">
        <FadeIn direction="up">
          <div className="mb-4">
            <Link href="/blog" className="text-[10px] uppercase tracking-[0.2em] text-brandRed font-bold hover:opacity-70 transition-opacity">
              ← Tüm Yazılar
            </Link>
          </div>
          <h1 className="text-4xl md:text-7xl font-display uppercase tracking-wider mb-4">
            {category.title}
          </h1>
          {category.description && (
            <p className="text-[10px] md:text-xs uppercase tracking-[0.4em] opacity-50 px-4 max-w-2xl mx-auto">
              {category.description}
            </p>
          )}
        </FadeIn>
        ...
        {posts?.length > 0 ? (
          ...
          <FadeIn>
            <div className="flex flex-col items-center justify-center py-32 border border-dashed border-gray-200 rounded-lg">
                ...
                <p className="text-primary/40 uppercase tracking-[0.2em] text-xs">Bu kategoride henüz yayınlanmış bir yazı bulunmuyor.</p>
            </div>
          </FadeIn>
        )}
      </div>
